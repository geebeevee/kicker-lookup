...
// Standard results table used across all panels
...
async function loadData() {
  ...
  // Round name lookup: round ID → round name
  const roundNameMap = {};
...
function renderLeague(league) {
  ...
  // League top 3 submitters by total points received
  ...
  // High + low score cards side by side
  ...
  // ── Rounds with fixed grid layout + accordion scoreboards ────────────────
  html += `<div class="dd-section-title">Rounds — click a round name to see full scoreboard</div>
  <div class="rounds-grid-header">
    <span class="rg-num">#</span>
    <span class="rg-name">Round</span>
    <span class="rg-desc">Theme</span>
    <span class="rg-winner">🏆 Winner</span>
    <span class="rg-link"></span>
  </div>`;
  ...
  leagueRounds.forEach((r, i) => {
    ...
    const plLink = playlistUrl
      ? `<a href="${esc(playlistUrl)}" target="_blank" class="playlist-link" onclick="event.stopPropagation()">▶ Playlist</a>`
      : '';
    ...
    if (winner) {
      ...
      winHtml = `<a href="summary.html?${wParams}" class="wiki-link" style="font-size:12px;" onclick="event.stopPropagation()">
        ${esc(getVal(winner,'Title'))} by ${esc(getVal(winner,'Artist(s)'))} — ${esc(wName)} (${winPts} pts)
      </a>`;
    }

    // Scoreboard rows sorted by pts desc
    const sbRows = [...roundSubs]
      ...
      .map(({ s, pts }, pos) => {
        ...
        const pillClass = pts >= 20 ? 'score-high' : pts <= 0 ? 'score-low' : 'score-mid';
        return `<tr>
          <td class="sb-pos">${pos + 1}</td>
          <td><a href="summary.html?${params}" class="wiki-link">${esc(title)}</a></td>
...
function buildLeagueMemberStats(league, leagueSubs, leagueVotes, playerMap) {
  ...
  const pairEntries = Object.entries(pairPts)
    ...
    .map(([key, pts]) => {
      ...
      return { voter: playerMap[vid] || vid, submitter: playerMap[sid] || sid, pts, count: pairCount[key] };
    });
  ...
  leagueVotes.forEach(v => {
    ...
    grpMap[key].sum   += num(getVal(v,'Points'));
    grpMap[key].count += 1;
  });
  ...
  leagueVotes.forEach(v => {
    ...
    const myPts      = num(getVal(v,'Points'));
    const othersMean = (grp.sum - myPts) / (grp.count - 1);
    if (!memberDevs[voterId]) { memberDevs[voterId] = 0; memberVoteCounts[voterId] = 0; }
    memberDevs[voterId]       += Math.abs(myPts - othersMean);
    memberVoteCounts[voterId] += 1;
  });
  ...
  function devRows(items, emoji) {
    return items.map((item,i) => `<tr>
      <td>${medal(i)}</td>
      <td>${emoji} <strong>${esc(item.name)}</strong></td>
      <td style="opacity:.6;font-size:12px;">avg deviation ${item.avg.toFixed(2)}</td>
    </tr>`).join('');
  }

  function fanRows(items) {
    return items.map((item,i) => {
      const cls = item.pts >= 0 ? 'vote-pts-pos' : 'vote-pts-neg';
      return `<tr>
        <td>${medal(i)}</td>
        <td><strong>${esc(item.voter)}</strong></td>
        <td style="opacity:.65;">→ ${esc(item.submitter)}</td>
        <td><span class="${cls}" style="font-weight:700;">${item.pts > 0 ? '+' : ''}${item.pts} pts</span></td>
        <td style="opacity:.5;font-size:12px;">${item.count} songs</td>
      </tr>`;
    }).join('');
  }
  ...
  <div class="member-stats-grid">
    <div class="ms-card">
      <h4>🐑 Vote With The Herd</h4>
      <p style="font-size:12px;opacity:.6;margin-bottom:8px;">Closest to group consensus this league</p>
      ${resultsTable(devRows(herd,'🐑'), '<th></th><th>Member</th><th>Avg deviation</th>')}
    </div>
    <div class="ms-card">
      <h4>🐺 Lone Wolves</h4>
      <p style="font-size:12px;opacity:.6;margin-bottom:8px;">Most diverged from group consensus</p>
      ${resultsTable(devRows(wolves,'🐺'), '<th></th><th>Member</th><th>Avg deviation</th>')}
    </div>
    <div class="ms-card">
      <h4>❤️ I'm Your Biggest Fan</h4>
      <p style="font-size:12px;opacity:.6;margin-bottom:8px;">Most total points given to one person's songs (min 3 songs)</p>
      ${resultsTable(fanRows(topFans), '<th></th><th>Voter</th><th>→ Artist</th><th>Total</th><th>Songs</th>')}
    </div>
    <div class="ms-card">
      <h4>💔 Not For Me</h4>
      <p style="font-size:12px;opacity:.6;margin-bottom:8px;">Lowest total points given to one person's songs (min 3 songs)</p>
      ${resultsTable(fanRows(notForMe), '<th></th><th>Voter</th><th>→ Artist</th><th>Total</th><th>Songs</th>')}
    </div>
  </div>`;
}
...
function getMemberIds(name) {
  return Object.entries(DATA.playerMap)
...
function renderMember(name) {
  ...
  const allAliasNames = memberIds.flatMap(id => nameHistory[id] || []);
  const uniqueAliases = [...new Set(allAliasNames)];
  const aliasHtml = uniqueAliases.length > 1
    ? uniqueAliases.filter(n => n !== name).map(n => `<span class="alias-badge">also known as: ${esc(n)}</span>`).join(' ')
    : '';
  ...
  // All-time rank among all submitters
  const allSubPts = {};
  subs.forEach(s => {
    const id  = clean(getVal(s,'Submitter ID'));
    const pts = totalPts[clean(getVal(s,'Spotify URI'))] ?? 0;
    allSubPts[id] = (allSubPts[id] || 0) + pts;
  });
  ...
  let html = `<div class="dd-card">
    <h3>${esc(name)} ${aliasHtml}</h3>
  ...
  [...memberSubs].sort((a,b)=>num(getVal(a,'League'))-num(getVal(b,'League'))).forEach(s => {
    ...
    html += `<div class="timeline-row">
      ...
      <span class="tl-title">
        <a href="summary.html?${params}" class="wiki-link"><strong>${esc(getVal(s,'Title'))}</strong></a>
  ...
  // Scoring History table — votes cast, sorted by league then round
  if (memberVotes.length) {
    // Build round order map: round_id → round index within its league
    const roundOrderMap = {};
    ...
    const sortedVotes = [...memberVotes].sort((a,b) => {
      ...
      const ra = roundOrderMap[clean(getVal(a,'Round ID'))] ?? 999;
      const rb = roundOrderMap[clean(getVal(b,'Round ID'))] ?? 999;
      return ra - rb;
    });

    const voteRows = sortedVotes.map(v => {
      const uri       = clean(getVal(v,'Spotify URI'));
      const sub       = DATA.subByURI[uri];
      const title     = sub ? getVal(sub,'Title')      : '(unknown)';
      const artist    = sub ? getVal(sub,'Artist(s)')  : '';
      const submitter = sub ? (playerMap[clean(getVal(sub,'Submitter ID'))] || '?') : '?';
      const vLeague   = clean(getVal(v,'League'));
      const vRoundId  = clean(getVal(v,'Round ID'));
      ...
      return `<tr>
        <td>${params ? `<a href="summary.html?${params}" class="wiki-link">${esc(title)}</a>` : esc(title)}</td>
        <td style="opacity:.65;">${esc(artist)}</td>
        <td style="opacity:.65;">${esc(submitter)}</td>
        <td><span class="score-pill ${pillClass}">${pts}</span></td>
...
function renderHoF() {
  ...
  const controversial = subs.map(s => {
    ...
    const vs   = (votesByURI[uri]||[]).map(v => num(getVal(v,'Points')));
  ...
  function songRow(s, pts, i, extra='') {
    ...
    return `<tr>
      ...
      <td>${extra || `<span class="score-pill ${scoreClass(pts)}">${pts} pts</span>`}</td>
    </tr>`;
  }
  ...
  <div class="dd-card">
    ...
    ${resultsTable(ranked.slice(0,20).map(({s,pts},i)=>songRow(s,pts,i)), '<th></th><th>Song</th><th>Score</th>')}
  </div>
  <div class="dd-card">
    ...
    ${resultsTable([...ranked].reverse().slice(0,10).map(({s,pts},i)=>songRow(s,pts,i)), '<th></th><th>Song</th><th>Score</th>')}
  </div>
  <div class="dd-card">
    ...
    )), '<th></th><th>Song</th><th>Spread / Score</th>')}
  </div>`;
...
function tryRenderH2H({ a, b }) {
  ...
  if (a.toLowerCase()===b.toLowerCase()) {
    document.getElementById('h2hContent').innerHTML = `<p class="dd-empty">Pick two different members.</p>`;
    return;
  }
  renderH2H(a, b);
}
function renderH2H(nameA, nameB) {
  ...
  if (!idsA.length || !idsB.length) {
    document.getElementById('h2hContent').innerHTML = `<p class="dd-empty">Could not find one or both members.</p>`;
    return;
  }
  ...
  const leaguesA = new Set(subsA.map(s=>clean(getVal(s,'League'))));
  const leaguesB = new Set(subsB.map(s=>clean(getVal(s,'League'))));
  const sharedLeagues = [...leaguesA].filter(l=>leaguesB.has(l)).sort((a,b)=>num(a)-num(b));
  ...
  let html = `<div class="dd-card" style="margin-bottom:14px;">
    <p style="opacity:.6;font-size:13px;">Shared leagues: ${sharedLeagues.length?sharedLeagues.map(l=>`<strong>L${l}</strong>`).join(', '):'None'}</p>
  </div>
  <div class="h2h-grid">
  ...
  function voteTable(votes, label) {
    ...
    const rows = [...votes].sort((a,b)=>num(getVal(b,'Points'))-num(getVal(a,'Points'))).map(v => {
      const uri    = clean(getVal(v,'Spotify URI'));
      const sub    = subByURI[uri];
      const title  = sub?getVal(sub,'Title'):'(unknown)';
      const artist = sub?getVal(sub,'Artist(s)'):'';
      const league = sub?clean(getVal(sub,'League')):'';
      const pts    = num(getVal(v,'Points'));
      const params = sub?new URLSearchParams({song:title,artist,league}):null;
      const pillClass = pts >= 4 ? 'score-high' : pts <= 0 ? 'score-low' : 'score-mid';
      return `<tr>
        <td>${params?`<a href="summary.html?${params}" class="wiki-link"><strong>${esc(title)}</strong></a>`:esc(title)}</td>
        <td style="opacity:.65;">${esc(artist)}</td>
...
function renderArtistStats() {
  ...
  const artistData = {};
  subs.forEach(s => {
    const artist = clean(getVal(s,'Artist(s)'));
    const uri    = clean(getVal(s,'Spotify URI'));
    if (!artist) return;
    if (!artistData[artist]) artistData[artist] = { uris: new Set(), subs: [], leagues: new Set() };
    artistData[artist].uris.add(uri);
    artistData[artist].subs.push(s);
    artistData[artist].leagues.add(clean(getVal(s,'League')));
  });
  ...
  const artists = Object.entries(artistData).map(([name, d]) => {
    ...
    const avg       = uniquePts.length ? Math.round((totalSum/uniquePts.length)*10)/10 : 0;
    return { name, count: d.subs.length, uniqueSongs: d.uris.size, pts: totalSum, avg, leagues: d.leagues.size, subs: d.subs };
  });
  ...
  function artistRows(list, showAvg=false) {
    return list.slice(0,20).map((a,i)=>{
      ...
      const subLinks = a.subs.slice(0,3).map(s=>{
        const p = new URLSearchParams({song:getVal(s,'Title'),artist:a.name,league:getVal(s,'League')});
        return `<a href="summary.html?${p}" class="wiki-link" style="font-size:12px;">${esc(getVal(s,'Title'))}</a>`;
      }).join(', ')+(a.subs.length>3?` +${a.subs.length-3} more`:'');
      return `<tr>
        <td>${medal(i)}</td>
        <td><strong>${esc(a.name)}</strong><br>
          <span style="font-size:12px;opacity:.6;">${subLinks}</span>
          ${!showAvg?`<div class="art-bar-wrap"><div class="art-bar" style="width:${barW}%"></div></div>`:''}
        </td>
        <td>${showAvg
          ?`<span class="score-pill ${scoreClass(a.avg*3)}">${a.avg} avg</span>`
          :`<span class="score-pill score-mid">${a.count}×</span>`}
        </td>
        <td style="opacity:.5;font-size:13px;">${a.leagues} league${a.leagues>1?'s':''}</td>
      </tr>`;
    }).join('');
  }
  ...
  <div class="dd-card">
    ...
    <p style="font-size:13px;opacity:.65;margin-bottom:12px;">Artists entered the most times across all leagues</p>
    ${resultsTable(artistRows(byCount), '<th></th><th>Artist</th><th>Count</th><th>Leagues</th>')}
  </div>
  <div class="dd-card">
    <h3>👑 Highest Avg Score (2+ unique songs)</h3>
    <p style="font-size:13px;opacity:.65;margin-bottom:12px;">Artists that consistently score well — averaged across unique songs only</p>
    ${resultsTable(artistRows(byAvg,true), '<th></th><th>Artist</th><th>Avg score</th><th>Leagues</th>')}
  </div>
  <div class="dd-card">
    <h3>📉 Lowest Avg Score (2+ unique songs)</h3>
    <p style="font-size:13px;opacity:.65;margin-bottom:12px;">Brave choices that never quite land</p>
    ${resultsTable(artistRows(byLow,true), '<th></th><th>Artist</th><th>Avg score</th><th>Leagues</th>')}
  </div>`;
